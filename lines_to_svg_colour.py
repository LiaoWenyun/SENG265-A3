...
def parse_line(line):
	...
	while True:
		...
		if m:
			...
			if offset == len(line): # no more tokens remaining
				if len(L) < 5: # error: too few x/y values
					return offset
				else: # use defualt colour
					L.append('Black')
					return L
